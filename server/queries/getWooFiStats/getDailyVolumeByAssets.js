...
function buildQuery({ chain }) {
  const query = `
    SELECT a.date, sum(a.volume) as volume, a.token_contract, sum(a.number_of_trades) as number_of_trades, a.chain, t.symbol, t.logo_url FROM (
      SELECT "date"::date::text, sum(usd_volume) as volume, from_token as token_contract, count(*) as number_of_trades, chain FROM woofi_swaps_${chain} GROUP BY token_contract, chain, date::date
      UNION ALL
      SELECT "date"::date::text, sum(usd_volume) as volume, to_token as token_contract, count(*) as number_of_trades, chain FROM woofi_swaps_${chain} GROUP BY token_contract, chain, date::date
    ) as a
