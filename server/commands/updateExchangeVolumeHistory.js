...
module.exports = async function updateExchangeVolumeHistory({ exchangeId, forceUpdate }) {
  await updateTokenPriceHistory({ tokenId: 'bitcoin' })
  const bitcoinHistory = await getTokenPriceHistory({ tokenId: 'bitcoin' })

  const volumeHistory = await fetchExchangeVolumeHistory({ exchangeId, forceUpdate })
